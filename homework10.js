const countriesData = require('./countries_data')

const countries = ['Finland', 'Sweden', 'Norway']

const nSet = new Set()

for(i=0;i<11;i++) {
        nSet.add(i)
    }
console.log(nSet)

nSet.delete(5)
nSet.clear()
console.log(nSet)

const arrStr = ['it','is','the','string','array']
const sSet = new Set(arrStr)
console.log(sSet)


const cMap = []   
countries.map(item => {
    cMap.push({Name: item, length: item.length})
})
console.log(cMap)

///level2

const a = [4, 5, 8, 9]
const b = [3, 4, 5, 7]

let c = [...a, ...b]
const union = new Set(c)
console.log(union)

let A = new Set(a)
let B = new Set(b)

let c = a.filter((num) => B.has(num))
let C = new Set(c)
console.log(C)

///////Find a with b?????????

////////////////////////////////////////level3

const arr = []
countriesData.map(item => {
    item.languages.forEach(elm => arr.push(elm))
})
const allLangs = new Set(arr)
console.log(allLangs.size)

const newArr = []                                                /////DAHA DERLİ TOPLU NASIL OLURDU ???????
for(const lang of allLangs) {
    const count = arr.filter(item => item == lang)
    newArr.push({Lang: lang , Count : count.length})
}
const sorted = newArr.sort((a,b) => {
    if(a.Count< b.Count) return 1
    if(a.Count> b.Count) return -1
    return 0
}  )
console.log(sorted.filter(item => sorted.indexOf(item)<10))